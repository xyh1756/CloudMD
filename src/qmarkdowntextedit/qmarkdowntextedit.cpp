...
void QMarkdownTextEdit::generatePdf(QString fileName)
{
    generateHtml(fileName);
    QString name="wkhtmltopdf "+fileName+".html "+fileName.remove(QRegExp("(\\.html)"))+".pdf";
    system(name.toStdString().data());
}

void QMarkdownTextEdit::generateHtml(QString fileName)
{
    QFile file(fileName);
    file.open(QFile::WriteOnly | QFile::Text);
    QTextStream out(&file);
    QApplication::setOverrideCursor(Qt::WaitCursor);
    QString text="<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\" /><script src=\"http://libs.baidu.com/jquery/2.0.0/jquery.min.js\"></script> <script src=\"https://cdn.bootcss.com/showdown/1.3.0/showdown.min.js\"></script> <script type=\"text/javascript\"> var content =\""+toPlainText().replace(QRegularExpression("\r|\n")," \\n")+"\";\n var converter = new showdown.Converter();\n var htmlcontent  = converter.makeHtml(content);\n document.write(htmlcontent);\n </script><style type=\"text/css\"> <!-- @import url(http://47.107.50.253/upload/static/1.css); --> </style>";
    out<<text;
...
/*void QMarkdownTextEdit::generateHtml(QString fileName)
{
    QFile scriptFile("3.js");
    scriptFile.open(QIODevice::ReadOnly);
    QTextStream out1(&scriptFile);
    QString contents=out1.readAll();
    scriptFile.close();
    QString text=toPlainText().replace(QRegularExpression("\r|\n")," \\n");
    QScriptEngine engine;
    QScriptValue js=engine.evaluate(contents);
    QScriptValue generate=engine.globalObject().property("generate");
    QScriptValueList args;
    args<<QScriptValue(text);
    QString result=generate.call(QScriptValue(),args).toString();
    QFile file(fileName);
    file.open(QFile::WriteOnly | QFile::Text);
